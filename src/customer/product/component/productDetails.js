...
import { fetchByIdAsync } from "../productSlice";
  ...
  const handleCart = (e) => {
    ...
    if (!productSize) {
      ...
      const productNew = {
        ...
        price: Discount(product.price, product.discountPercentage),
        size: productSize,
      };
      dispatch(addToCartAsync({ ...productNew, quantity: 1, userid: user.id }));
